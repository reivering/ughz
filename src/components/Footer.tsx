...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-[#111] text-[#fcfcfc] px-6 py-24 md:px-12 md:py-32 flex flex-col justify-between relative overflow-hidden z-20 shadow-[0_-20px_60px_-15px_rgba(0,0,0,0.8)]" id="contact">
      
      {/* Background ambient glow */}
      <div className="absolute bottom-0 right-0 w-[50vw] h-[50vw] bg-[#FF661A]/10 mix-blend-screen blur-[150px] rounded-full pointer-events-none z-0" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-8 mb-32 z-10 relative">
        <motion.div style={{ y: ctaY }} className="flex flex-col">
          ...
          <p className="text-lg md:text-xl text-[#a0a0a0] max-w-md text-balance mb-8">
           ...
           <form action="https://formspree.io/f/mykdbrqv" method="POST" className="flex flex-col gap-8 w-full max-w-lg">
              <div className="flex flex-col gap-2 relative group">
                 <input 
                   ...
                   required
                   className="w-full bg-transparent border-b border-[#333] py-4 text-lg md:text-xl focus:outline-none focus:border-[#FF661A] transition-colors peer placeholder:text-[#666]" 
                 />
                 <div className="absolute bottom-0 left-0 w-0 h-[2px] bg-[#FF661A] transition-all duration-500 peer-focus:w-full" />
              </div>
              ...
              <div className="flex flex-col gap-2 relative group mt-4">
                 <textarea 
                   ...
                   rows={3}
                   required
                   className="w-full bg-transparent border-b border-[#333] py-4 text-lg md:text-xl focus:outline-none focus:border-[#FF661A] transition-colors peer resize-none placeholder:text-[#666]" 
                 />
                 <div className="absolute bottom-0 left-0 w-0 h-[2px] bg-[#FF661A] transition-all duration-500 peer-focus:w-full" />
              </div>

              <button type="submit" className="mt-4 self-start bg-white text-[#111] px-8 py-4 rounded-full font-medium hover:bg-[#FF661A] hover:text-white transition-colors duration-300">
                 Send Message →
              </button>
           </form>
        </motion.div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-t border-[#333] pt-8 gap-8 z-10 relative">
        ...
        <div className="flex gap-6 text-sm text-[#666] flex-col md:flex-row items-start md:items-center">
          ...
          <span className="mt-4 md:mt-0 flex items-center gap-2">
            © {new Date().getFullYear()} zeyno <span className="w-1.5 h-1.5 bg-[#FF661A] rounded-full inline-block animate-[ping_2s_cubic-bezier(0,0,0.2,1)_infinite]" />
          </span>
        </div>
      </div>
    </footer>
