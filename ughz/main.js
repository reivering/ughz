...
document.addEventListener('click', (e) => {
  ...
  if (link &&
    ...
    // Check if it's the same page (avoid transitions for same-page anchors)
...
// Force reveal if preloader hangs or for fast connections
window.addEventListener('load', () => {
  setTimeout(() => {
    ScrollTrigger.refresh();
  }, 100);
...
function initHeroReveal() {
  ...
  // Check if it's the home page or a case study page
...
/* ============================================================
   10. SCROLL ANIMATIONS (init after preloader)
   ============================================================ */
function initScrollAnimations() {
  ...
  // ── Process-cards — Horizontal scroll ──
  ...
  if (track && trackWrap && window.innerWidth > 768) {
    const tl = gsap.timeline({
      scrollTrigger: {
        trigger: '#process',
        start: 'top top',
        end: () => `+=${track.scrollWidth - trackWrap.offsetWidth + 600}`,
        pin: true,
        scrub: 1,
